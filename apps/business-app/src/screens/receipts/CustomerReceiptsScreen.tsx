...
import { useCustomerReceipts } from '@/hooks/useApi';
import { formatDate, formatMoney } from '@/utils/format';
import { RootStackParamList } from '@/navigation/types';

export function CustomerReceiptsScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'CustomerReceipts'>>();
  const { data: receipts, isLoading, isError, error, refetch } = useCustomerReceipts(route.params.customerId);
  ...
  if (!receipts?.length) return <EmptyState title="No receipts yet" subtitle="Receipts appear here after each collected payment." />;
            ...
            <View style={styles.row}>
              <View>
                ...
                <Text style={styles.caption}>
                  {formatDate(item.createdAt)} · {item.collectorLabel}
                </Text>
              </View>
